...
## Derin Temizlik düğmesi

Belgeler penceresindeki **Derin Temizlik** düğmesi verilen Belgeler
alanlarının alt klasörlerine kadar iner. Şunları yapar:

- birleşik bilimsel proje koleksiyonunu `PROJE_ARSIVI` altında tutar,
- bağımsız PDF, Excel, EXE, görsel ve betikleri köken/tür klasörlerine taşır,
- GitHub depolarını kopyalamadan `PROJE_ARSIVI\GitHub_Baglantilari` altında
  erişilebilir yapar,
- `__pycache__`, `.pyc`, sanal ortam ve benzeri yeniden üretilebilir
  artıkları kaldırır,
- yeni dosya-kökenli arşivdeki SHA-256 eşleşen kopyaları tekilleştirir.

Kaynak `.py` dosyaları, `.git` içeriği ve GitHub çalışma kopyaları korunur.
Düğme gerçek işlemden önce kullanıcı onayı ister.
