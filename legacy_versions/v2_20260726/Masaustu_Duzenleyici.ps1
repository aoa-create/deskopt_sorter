...
function Get-DesktopPlan {
    ...
    foreach ($item in $items) {
        ...
        $destinationDirectory = Join-Path `
            (Join-Path $script:DestinationRoot $project) `
            $typeFolder
...
function Write-RunFiles {
    ...
    $logRoot = Join-Path $script:DestinationRoot "_ISLEM_KAYITLARI"
...
function Format-PlanText {
    ...
    foreach ($entry in $Plan) {
        $lines.Add(
            ("{0} -> {1}\{2}" -f `
                $entry.Name, $entry.Project, $entry.TypeFolder)
        )
    }
...
function Start-OrganizerGui {
    ...
    $description.Text = (
        "Dosyaları Belgeler\Masaustu_Duzenlenmis altında proje ve türüne " +
        "göre taşır. Hiçbir dosyayı silmez."
    )
